...
TenderGraph independently reimplements the harness engineering pattern described in *From Prompts to Contracts: Harness Engineering for Auditable Enterprise LLM Agents*. The adoption target is a traceable application-level harness: deterministic product behavior lives in schemas, policies, validation gates, and trace artifacts rather than prompt prose. The paper does not define a numbered maturity level, so TenderGraph does not claim one.
...
| `SourceManifest` | Indirectly | Identity, hash, source status, runtime policy, issuer, selection rule, license, version, retrieval mode |
...
1. Bind the explicit fixture route to jurisdiction, procedure, and lot, then infer intent. Free-form entity resolution outside the bounded fixture inventory is not implemented.
...
The trace records six ordered stages: entity routing, source collection, claim selection, answer planning, composition, and output validation. Trace validation checks the exact order, stage artifacts, and agreement between the routing stage and the resolved procedure scope.
...
`src/cli/codex-run.ts` is the Build Week live-model boundary. It invokes ChatGPT-authenticated Codex with `gpt-5.6-terra`, a read-only sandbox, and a strict JSON Schema. Application code creates the exact input contract before the model run and validates the candidate afterward. The trace records the Codex session ID, model, composition surface, gates, and timing. Raw candidates are deleted after validation; the retained candidate record contains only a hash, character count, and schema-validity status.
...
| Source integrity and eligibility | `BROKEN_SOURCE_LINK` / `INELIGIBLE_SOURCE` | Block or fallback |
| Scope isolation | `SCOPE_CONTAMINATION` | Block or fallback |
| Answer completeness | `MISSING_EXPECTED_CLAIM` | Fallback |
| Output hygiene and decision language | `INTERNAL_CONTENT_LEAKAGE` / `UNBOUNDED_DECISION_LANGUAGE` | Block and fallback |
| Trace completeness | `INCOMPLETE_TRACE` | Operational failure |
| Latency budget | `LATENCY_BUDGET_EXCEEDED` | Record and degrade |

Reader output never contains system prompts, hidden policies, secrets, internal trace content, or unbounded bid/no-bid instructions. The trace records model, mode, contract version, scope, sources, evidence, claims, ordered runtime stages, validation results, fallback reason, and timings.

## 7. Evaluation Protocol

The repository includes 23 fixed scenarios: 17 Chile, 3 TED, and 3 Contracts Finder. Each declares the question, fixture, expected claim IDs, and expected status. The deterministic suite is the reproducible baseline; live GPT-5.6 repetitions can be layered on without changing expected assertions.

Eight one-property fault injections verify enforcement:

1. Missing expected claim.
2. Wrong procedure scope.
3. Missing answer signal.
4. Internal-content leakage.
5. Unknown evidence reference.
6. Unpromoted claim.
7. Invented claim.
8. Latency over budget.

This credential-free ablation compares the code-owned harness with schema-only admission. It is not the paper's live-model RQ3 experiment against prompt-only and an external guardrail.
...
1. Expand the hash-verified Chile snapshot corpus and complete independent manual labels.
2. Add live connector adapters while retaining immutable snapshot fallback and recorded source states.
...
5. Repeat all 23 live scenarios three times and publish stability, completeness, refusal, and latency results.
6. Run the paper-style same-model harness-versus-prompt-only-versus-guardrail ablation with paired adversarial scenarios and a utility measure.
7. Run a genuine multi-model composition-boundary experiment before making an RQ2 model-substitution claim.
8. Record the three-minute demo from the deployed, tested build.
...
The exact paper-to-runtime mapping and non-replicated experiments are recorded in `docs/PAPER_ADOPTION_AUDIT.md`.
